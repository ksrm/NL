...
for(p in 1:length(text)){
  ...
  for(i in 1:num_words){
    ...
    number_codes[i] <- which(dictionary == test_text[i] )
...
word_to_number <- function(word,dictionary){
  which(dictionary == word)
}

word_to_number("ago",dictionary)

unlist(lapply(test_text, function(x) {which(dictionary == x)}))

library(magrittr)

lapply(test_text, function(x) {which(dictionary == x)}) %>%
  unlist() %>%
  paste(collapse=" ")

paste(unlist(lapply(test_text, function(x) {which(dictionary == x)})),collapse=" ")
...
doc_term_matrix <- matrix(0,ncol=length(dictionary),
                          nrow=length(text)) 

colnames(doc_term_matrix) <- dictionary

test_text <- text[1]
test_text <- unlist(strsplit(test_text, split=" "))

word_counts <- table(test_text)
attributes(word_counts)
names(word_counts)
word_counts

doc_term_matrix[1,names(word_counts)] <- word_counts
